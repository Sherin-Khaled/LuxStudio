...
type GalaxyParticleKind = "core" | "band" | "lane" | "star" | "micro";
...
interface GalaxyParticle {
  ...
  size: number;
  opacity: number;
  color: string; // for "band"/"lane": a sprite key; for "core"/"star"/"micro": a literal CSS color
...
const GALAXY_SPRITE_RGB: Record<string, string> = {
  warm: "255,247,232",
  cyan: "170,226,255",
  blue: "120,158,255",
  violet: "160,120,255",
...
function buildGalaxySprites(): Record<string, HTMLCanvasElement> {
  const sprites: Record<string, HTMLCanvasElement> = {};
  for (const key of Object.keys(GALAXY_SPRITE_RGB)) {
    sprites[key] = createGalaxySprite(GALAXY_SPRITE_RGB[key]);
  }
  return sprites;
}
...
function generateGalaxyParticles(isMobile: boolean): GalaxyParticle[] {
  ...
  const starCount = isMobile ? 150 : 380;
  const microCount = isMobile ? 130 : 300;
  ...
  const coreColors = ["#fff8ee", "#eaf6ff", "#cfe9ff"];
  ...
  for (let arm = 0; arm < GALAXY_ARM_COUNT; arm++) {
    ...
    for (let s = 0; s < armSteps; s++) {
      ...
      for (let p = 0; p < perStep; p++) {
        const perpJitter = (Math.random() - 0.5) * (0.05 + t * 0.1); // organic thickness, not a clean line
  ...
  // 3. Dark dust lanes — same systematic walk, offset just inside/trailing
  // each bright arm, dark color, low opacity — real contrast, not harsh lines
  for (let arm = 0; arm < GALAXY_ARM_COUNT; arm++) {
    const armOffset = (arm / GALAXY_ARM_COUNT) * Math.PI * 2;
    for (let s = 0; s < laneSteps; s++) {
      ...
      for (let p = 0; p < lanePerStep; p++) {
        ...
        particles.push({
          ...
          opacity: Math.max(0.05, 0.3 - t * 0.18) * (0.7 + Math.random() * 0.4),
          color: "dust",
          kind: "lane",
        });
      }
  ...
  const starColors = ["#ffffff", "#dff0ff", "#9ce0ff", "#38bdf8", "#c9b8ff"];
  for (let i = 0; i < starCount; i++) {
    ...
    particles.push({
      ...
      opacity: Math.max(0.15, (1 - t) * 0.7 + Math.random() * 0.25),
  ...
  for (let i = 0; i < microCount; i++) {
    ...
    particles.push({
      ...
      opacity: Math.max(0.03, (1 - t) * 0.28 + Math.random() * 0.1),
      color: "#c9e6ff",
      kind: "micro",
    });
  }
...
function drawGalaxyFrame(
  ...
  const coreGradient = ctx.createRadialGradient(0, 0, 0, 0, 0, coreRadius * 3.2);
  ...
  // Spiral arm bands — soft sprites stamped densely along each arm, additive
  // blending so overlapping stamps brighten into a continuous glowing band.
  ...
  // Dark dust lanes — normal blending (not additive) so they actually darken
  // the bright band underneath, instead of brightening it further.
...
const AnimatedGalaxy = ({ reduced }: { reduced: boolean }) => {
  ...
  const particlesForMobileRef = useRef<boolean | null>(null);
  if (particlesRef.current === null || particlesForMobileRef.current !== isMobile) {
    particlesRef.current = generateGalaxyParticles(isMobile);
    particlesForMobileRef.current = isMobile;
  }

  const spritesRef = useRef<Record<string, HTMLCanvasElement> | null>(null);
  if (!spritesRef.current) {
    spritesRef.current = buildGalaxySprites();
  }
  ...
  useEffect(() => {
    ...
    function renderStatic() {
      drawGalaxyFrame(ctx!, canvas!.width, canvas!.height, particlesRef.current!, sprites, 0, 0, true);
    }

    function loop() {
      rotation += GALAXY_ROTATION_PER_FRAME;
      pulsePhase += 0.012;
      drawGalaxyFrame(ctx!, canvas!.width, canvas!.height, particlesRef.current!, sprites, rotation, pulsePhase, false);
  ...
  return (
    <div
      ...
      role="img"
      aria-label="An animated spiral galaxy made of particles, representing design, code, and connected digital systems"
    >
      <style>{`
...
export const AboutPage = (): JSX.Element => {
  ...
  return (
    <div className="relative min-h-screen w-full text-[#f5f7fa] overflow-x-hidden">
        ...
        <div className="relative z-10 mx-auto w-full max-w-[1300px] px-6 sm:px-8 lg:px-12">
          <div className="flex flex-col gap-12 lg:flex-row lg:items-center lg:gap-16">
            ...
            <div className="flex-1">
              ...
              >
                Creative Technology Studio Â· Design Â· Systems Â· Launch
              </motion.p>
            </div>
            ...
            >
              <AnimatedGalaxy reduced={reduced} />
            </motion.div>
          </div>
        </div>
      </section>
      ...
      <section className="relative flex min-h-[560px] w-full items-center justify-center overflow-hidden sm:min-h-[640px] lg:min-h-[720px]" style={{ backgroundColor: "#020409" }}>
        {/* Top fade — blends the hero's background color (#03050a) into this
            section's slightly different tone (#020409) so there is no visible
            horizontal seam where the two sections meet. */}
        <div
          aria-hidden="true"
          className="pointer-events-none absolute inset-x-0 top-0 z-[3] h-[160px]"
          style={{ background: "linear-gradient(to bottom, #03050a 0%, rgba(3,5,10,0) 100%)" }}
        />

        {/* Sparse background stars — reuses the existing StarsBackground component, low count on purpose */}
        <StarsBackground count={30} mobileCount={14} tabletCount={22} motion="twinkle" className="z-[1]" />

        {/* Subtle vertical grid lines */}
        <div
          aria-hidden="true"
          className="pointer-events-none absolute inset-0 z-[1]"
          style={{ backgroundImage: "repeating-linear-gradient(90deg, rgba(255,255,255,0.018) 0px, rgba(255,255,255,0.018) 1px, transparent 1px, transparent 120px)" }}
        />

        {/* Violet corner glows — the two-tone edge effect (blue/cyan near the arc, violet further out) */}
        <div aria-hidden="true" className="pointer-events-none absolute bottom-[-12%] left-[-10%] z-[1] h-[380px] w-[380px] rounded-full blur-[120px]" style={{ backgroundColor: "rgba(124,58,237,0.18)" }} />
        <div aria-hidden="true" className="pointer-events-none absolute bottom-[-12%] right-[-10%] z-[1] h-[380px] w-[380px] rounded-full blur-[120px]" style={{ backgroundColor: "rgba(124,58,237,0.18)" }} />
        ...
        >
          ...
          <ellipse cx="600" cy="770" rx="620" ry="360" fill="url(#philosophy-arc-pool)" filter="url(#philosophy-arc-blur-wide)" />
          ...
          <ellipse
            ...
            filter="url(#philosophy-arc-blur-wide)"
          ...
          <ellipse
            ...
            filter="url(#philosophy-arc-blur-atmo)"
            opacity="0.72"
          ...
          <ellipse
            ...
            fill="none" stroke="#8b5cf6" strokeOpacity="0.32" strokeWidth="24"
          ...
          <ellipse
            ...
            fill="none" stroke="#38bdf8" strokeOpacity="0.32" strokeWidth="56"
            filter="url(#philosophy-arc-blur-med)"
          ...
          <ellipse
            ...
            fill="none" stroke="#bfe8ff" strokeOpacity="0.42" strokeWidth="16"
        ...
        {/* Bottom fade — dissolves the arc/glow into solid dark before the next
            section starts, so there is no sharp horizontal cut at the seam. */}
        <div
          aria-hidden="true"
          className="pointer-events-none absolute inset-x-0 bottom-0 z-[3] h-[200px]"
          style={{ background: "linear-gradient(to bottom, transparent 0%, #03050a 70%, #03050a 100%)" }}
        />

        <div className="relative z-10 mx-auto mb-24 w-full max-w-[860px] px-6 text-center sm:mb-32 sm:px-8 lg:mb-48">
          <motion.div
            variants={fadeUp} initial={reduced ? "visible" : "hidden"} whileInView="visible" viewport={{ once: true }}
            className="inline-flex items-center gap-2 rounded-full border border-white/[0.12] bg-white/[0.04] px-4 py-1.5"
          >
            <span className="h-1.5 w-1.5 shrink-0 rounded-full bg-[#38bdf8] shadow-[0_0_8px_rgba(56,189,248,0.8)]" />
            <span className="[font-family:'JetBrains_Mono',Helvetica] text-[10.5px] tracking-[2px] text-[#f5f7fa99] uppercase">
              Our Philosophy
            </span>
          </motion.div>
          <motion.h2
            variants={fadeUp} initial={reduced ? "visible" : "hidden"} whileInView="visible" custom={0.1} viewport={{ once: true }}
            className="mt-7 [font-family:'Bricolage_Grotesque',Helvetica] text-[40px] font-medium leading-[1.05] tracking-[-1.2px] text-[#f5f7fa] sm:text-[52px] lg:text-[68px] lg:tracking-[-2.1px]"
          >
        ...
        <div className="relative z-10 mx-auto w-full max-w-[1000px] px-6 sm:px-8 lg:px-12">
          ...
          >
            <div
              ...
              className="studio-note-glow-border isolate relative overflow-hidden rounded-[28px] border border-white/[0.12] bg-white/[0.04] p-10 shadow-[0_40px_120px_rgba(0,0,0,0.5)] backdrop-blur-sm [transition:border-color_700ms_cubic-bezier(0.22,1,0.36,1),transform_700ms_cubic-bezier(0.22,1,0.36,1),box-shadow_700ms_cubic-bezier(0.22,1,0.36,1)] hover:-translate-y-1 hover:border-[rgba(56,189,248,0.45)] hover:shadow-[0_0_24px_rgba(56,189,248,0.20),0_0_80px_rgba(56,189,248,0.10),0_40px_120px_rgba(0,0,0,0.5)] lg:p-14"
            >
              {/* Top inset line */}
              ...
              <div aria-hidden="true" className="studio-note-scrim pointer-events-none absolute inset-0 z-[1] bg-[#03050a]/25" />
              {/* Studio signal dot */}
          ...
          >
            {studioLayers.map((word, i) => (
              <motion.span
                ...
                className="relative cursor-pointer select-none rounded-full border border-white/[0.10] bg-white/[0.04] px-4 py-2 [font-family:'Inter',Helvetica] text-[13px] font-normal text-[#f5f7fa99] [transition:color_500ms_cubic-bezier(0.22,1,0.36,1),border-color_500ms_cubic-bezier(0.22,1,0.36,1),background-color_500ms_cubic-bezier(0.22,1,0.36,1),box-shadow_500ms_cubic-bezier(0.22,1,0.36,1)] [will-change:transform,box-shadow] hover:z-[5] hover:border-[rgba(56,189,248,0.6)] hover:bg-[rgba(56,189,248,0.14)] hover:text-[#38bdf8] hover:shadow-[0_0_20px_rgba(56,189,248,0.35),0_0_50px_rgba(56,189,248,0.18),inset_0_1px_0_rgba(255,255,255,0.14)]"
              >
                {word}
              </motion.span>
            ))}
          ...
          <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
            {values.map((v, i) => (
              <motion.div
                ...
                tabIndex={0}
                style={{ transformOrigin: "center" }}
                className={`group relative overflow-hidden rounded-[20px] border border-white/[0.11] bg-white/[0.04] p-7 backdrop-blur-sm outline-none [transition:border-color_600ms_cubic-bezier(0.22,1,0.36,1),box-shadow_600ms_cubic-bezier(0.22,1,0.36,1)] hover:border-[rgba(56,189,248,0.45)] hover:shadow-[0_0_24px_rgba(56,189,248,0.18),0_0_70px_rgba(56,189,248,0.08)] focus-visible:border-[rgba(56,189,248,0.45)] focus-visible:shadow-[0_0_24px_rgba(56,189,248,0.18),0_0_70px_rgba(56,189,248,0.08)] ${i === 4 ? "sm:col-span-2 lg:col-span-1" : ""}`}
              >
                ...
                <div className="relative z-10">
                  <span className="[font-family:'JetBrains_Mono',Helvetica] text-[10.5px] tracking-[1.5px] text-[#38bdf880] [transition:color_600ms_cubic-bezier(0.22,1,0.36,1)] group-hover:text-[#38bdf8] group-focus-visible:text-[#38bdf8]">{v.num}</span>
                  <h3 className="mt-3 [font-family:'Bricolage_Grotesque',Helvetica] text-[20px] font-medium tracking-[-0.4px] text-[#f5f7fa] [transition:text-shadow_600ms_cubic-bezier(0.22,1,0.36,1)] group-hover:[text-shadow:0_0_16px_rgba(255,255,255,0.28)] group-focus-visible:[text-shadow:0_0_16px_rgba(255,255,255,0.28)]">{v.title}</h3>
                  <p className="mt-3 [font-family:'Inter',Helvetica] text-[13.5px] font-normal leading-[22px] text-[#f5f7faa6] [transition:color_600ms_cubic-bezier(0.22,1,0.36,1)] group-hover:text-[#f5f7fac2] group-focus-visible:text-[#f5f7fac2]">{v.desc}</p>
                </div>
              </motion.div>
            ))}
          </div>
              ...
              <div className="relative z-10 flex flex-col items-center text-center">
                ...
                <div className="mt-10 flex flex-wrap items-center justify-center gap-4">
                  <button
                    ...
                    data-testid="button-about-cta-start"
